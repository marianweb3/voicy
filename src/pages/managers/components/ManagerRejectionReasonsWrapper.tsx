...
import { useManagerView } from "../../../hooks/useManagers";
import { Dropdown } from "../../../components/ui/dropdown";
import RejectionReasonsChart from "../../dashboard/components/RejectionReasonsChart";
...
interface ManagerRejectionReasonsWrapperProps {
  title?: string;
...
const ManagerRejectionReasonsWrapper = ({
  title = "Причини відмов",
  defaultRange = "all",
  showInsight = true,
  showDropdown = true,
  className = "",
}: ManagerRejectionReasonsWrapperProps) => {
  ...
  const { managerData, isLoadingManagerView, managerViewError, timeUpdate } =
    useManagerView(managerId, selectedRange);
  ...
  const chartData: ChartDataItem[] = managerData?.rejects?.items
    ? managerData.rejects.items.map((item) => ({
        //@ts-ignore
        name: item.title,
        //@ts-ignore
        value: item.percent,
  ...
  const totalRejections = managerData?.rejects?.total || 0;
  const insightText = managerData?.rejects?.text || "";
...
export default ManagerRejectionReasonsWrapper;
