...
import { useDashboardBest } from "../../../hooks/useDashboard";
...
interface BestManagersWrapperProps {
  title?: string;
  className?: string;
}

const BestManagersWrapper = ({
  title = "Найкращі менеджери",
  className = "",
}: BestManagersWrapperProps) => {
  const [periodFilter, setPeriodFilter] = useState<"week" | "month" | "year">(
    "week"
  );

  const { bestManagers, isLoadingBest, bestError } =
    useDashboardBest(periodFilter);

  // Transform API data to component format
  const transformedData: ManagerData[] = bestManagers
    ? bestManagers.map((manager) => ({
  ...
  const LoadingSkeleton = () => {
    const barHeights = [75, 85, 60, 90, 70]; // Different heights for variety
  ...
  return (
    ...
    >
      ...
      {isLoadingBest ? (
        ...
        <ManagerPerformanceBars data={transformedData} isPositive={true} />
      )}
    </div>
  );
};

export default BestManagersWrapper;
